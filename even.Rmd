...
for(pos in parts_of_speech){
  ...
  for(i in seq(1, length(relevant_cols))){
    ...
    pvs_bundled <- ifelse(factor_status_bundled == factor_status$SUCC, unname(coef(summary(fit_random))[,'Pr(>|z|)'][2]), NA)
    pvs_random <- ifelse(factor_status_random == factor_status$SUCC, unname(coef(summary(fit_bundled))[,'Pr(>|z|)'][2]), NA)
...
for(factor in res$factor){
  ...
  current_col %>%
    filter(current_col[, 1] == 1) -> current_col
  num_of_occ <- nrow(current_col)
  current_col %>%
    count(corpus) %>%

...
get_cis <- function(table_for_CI){
  table_for_CI %>%
    mutate(CI_k_l = NA, CI_k_r = NA, CI_s_l = NA, CI_s_r = NA, diff = NA) -> table_for_CI
  ...
  for(i in seq(1, length(table_for_CI$factor))){
    ...
    tryCatch(
      ...
      pval <- unname(coef(summary(fit))[,'Pr(>|z|)'][2])
              ...
              table_for_CI$CI_k_l[i] <- unname(confint(fit, parm="beta_",method="Wald", level = 0.95)[1,])[1]
      table_for_CI$CI_k_r[i] <- unname(confint(fit, parm="beta_",method="Wald", level = 0.95)[1,])[2]
      table_for_CI$CI_s_l[i] <- unname(confint(fit, parm="beta_",method="Wald", level = 0.95)[2,])[1]
      table_for_CI$CI_s_r[i] <- unname(confint(fit, parm="beta_",method="Wald", level = 0.95)[2,])[2]
...
res <- read.csv("even_dialects - facts_res.csv")
...
res_random$diff <- abs(res_random$diff)
```
